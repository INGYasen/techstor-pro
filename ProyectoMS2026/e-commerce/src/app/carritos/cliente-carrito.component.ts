...
export class ClienteCarritoComponent implements OnInit, OnDestroy {
  ...
  protected readonly metodosPago = ['TARJETA', 'YAPE', 'EFECTIVO', 'TRANSFERENCIA'];
  ...
  confirmarPedido(): void {
    ...
    if (this.checkoutForm.invalid) {
      ...
      this.error.set('Completa la dirección de envío y el método de pago.');
      return;
    }
    ...
    const { direccionEnvio, observacion, metodo, referenciaTransaccion } = this.checkoutForm.getRawValue();
    ...
    this.pedidoService
        ...
        switchMap((pedido) =>
          this.pagoService
            .crear({
              ...
              metodo,
              estado: 'APROBADO',
              referenciaTransaccion: referenciaTransaccion || undefined,
            })
            .pipe(map(() => pedido)),
        ),
      )
      .subscribe({
        next: (pedido) => {
          ...
          void this.router.navigate(['/cliente/pagos'], {
            state: {
              mensaje: `¡Compra exitosa! Pedido #${pedido.id} pagado con ${metodo}.`,
              pedidoId: pedido.id,
            },
          });
        },
        error: (err) => {
          this.error.set(
            extractHttpErrorMessage(
              err,
              'No se pudo completar la compra. Si el pedido se creó sin pago, revisa Mis pedidos.',
            ),
          );
          this.processing.set(false);
        },
